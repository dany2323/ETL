...
public class SireHistoryProjectDAO {
	...
	public static void fillSireHistoryProject(long minRevision, long maxRevision)
		...
		try {
			...
			for (Tuple row : projects) {
				...
				if(!ProjectTemplateINI.existProjectTemplateIni(connOracle, row.get(fonteProjects.cLocation), row.get(fonteProjects.cRev), DmAlmConstants.REPOSITORY_SIRE)) {
					ProjectTemplateINI.fillProjectTemplateIniFile(connOracle, row.get(fonteProjects.cLocation), row.get(fonteProjects.cRev), DmAlmConstants.REPOSITORY_SIRE);
				}
			}
	...
	public static long getMinRevision() throws DAOException {
		...
		try {
			...
			QSireHistoryProject stg_Projects = QSireHistoryProject.sireHistoryProject;
			SQLTemplates dialect = new HSQLDBTemplates();
