	...
	public static void fillSireHistoryProject(long minRevision, long maxRevision)
		...
		try {
			...
			for (Tuple row : projects) {
				...
				String cCreated = vals[15] != null ? (queryConnOracle(connOracle, dialect).from(stgRevision).where(stgRevision.cName.eq(Long.valueOf(vals[15].toString()))).where(stgRevision.cRepo.eq(lispa.schedulers.constant.DmAlmConstants.REPOSITORY_SIRE)).count() > 0 ? queryConnOracle(connOracle, dialect).from(stgRevision).where(stgRevision.cName.eq(Long.valueOf(vals[15].toString()))).where(stgRevision.cRepo.eq(lispa.schedulers.constant.DmAlmConstants.REPOSITORY_SIRE)).list(stgRevision.cCreated).get(0).toString() : "") : "";
