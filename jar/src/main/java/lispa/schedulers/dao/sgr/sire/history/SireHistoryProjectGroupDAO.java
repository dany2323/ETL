		...
		try {
			...
			projectgroups = query.from(fonteProjectGroups)
					.list(
							...
							StringTemplate.create("(SELECT c.c_pk FROM " + lispa.schedulers.manager.DmAlmConstants.GetDbLinkPolarionCurrentSire() + " c WHERE c.c_id = " + fonteProjectGroups.fkUriParent + ") || '%' || c_rev as fk_parent"),
