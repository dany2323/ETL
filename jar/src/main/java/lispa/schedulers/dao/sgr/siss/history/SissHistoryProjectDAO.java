		...
		try {
			...
			projects = query
					...
					.list(
							...
							StringTemplate.create("(SELECT f.c_pk FROM " + lispa.schedulers.manager.DmAlmConstants.GetDbLinkPolarionCurrentSiss() + " f WHERE f.c_id = " + fonteProjects.fkUriLead + ") || '%' || project.c_rev as fk_lead"),
