		...
		try {
			...
			projects = query
					...
					.list(
							...
							StringTemplate.create("(SELECT e.c_pk FROM " + lispa.schedulers.manager.DmAlmConstants.GetDbLinkPolarionCurrentSiss() + " e WHERE e.c_id = " + fonteProjects.fkUriProjectgroup + ") || '%' || (select c_rev from " + lispa.schedulers.manager.DmAlmConstants.GetPolarionSchemaSissHistory() + ".projectgroup where projectgroup.c_pk = fk_projectgroup) as fk_projectgroup"),
