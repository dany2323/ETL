...
public class CheckLinkAsmSferaProjectFacade {
	private static Logger logger = Logger
			.getLogger(CheckLinkAsmSferaProjectFacade.class);
	private static QDmalmAsm dmalmAsm = QDmalmAsm.dmalmAsm;
	private static QDmalmProject project = QDmalmProject.dmalmProject;
	private static QDmalmAsmProject asmProject = QDmalmAsmProject.dmalmAsmProject;
	private static QDmalmProdotto p = QDmalmProdotto.dmalmProdotto;
	...
	public static void execute() throws Exception, DAOException {
		...
		try {
			...
			logger.info("START CheckLinkAsmSferaProjectFacade");
			...
			prod = query.from(p)
					.where(p.dtFineValidita.eq(DateUtils.setDtFineValidita9999()))
					.where(p.dmalmProdottoSeq.notIn(new SQLSubQuery().from(
							asmProject)
							.join(p).on(p.dmalmProdottoSeq.eq(asmProject.dmalmProdottoPk))
			...
			logger.info("STOP CheckLinkAsmSferaProjectFacade");
	...
	private static void insertAsm(DmalmProject pFound, Tuple ta, String sigla)
		...
		try {
			...
			r = ProdottoDAO.getProductBySigla(sigla, inizioVal, fineVal);
			...
			if (r.size() > 0) {
				...
				for (Tuple t : r) {
					...
					check = checkRelazione(t.get(p.dmalmProdottoSeq),
					...
					if (check.size() == 0) {
						...
						if (old.size() == 0) {
							...
							if (oldAsm.size() == 0) {
								...
								delete(t.get(p.dmalmProdottoSeq), 0,
										pFound.getDmalmProjectPk());
								insert(t.get(p.dmalmProdottoSeq),
										ta.get(dmalmAsm.dmalmAsmPk),
										pFound.getDmalmProjectPk());
		...
		try {
			...
			if (r.size() > 0 && tp.get(project.dmalmProjectPrimaryKey) != 0) {
				toCheck = checkRelazione(
						r.get(0).get(p.dmalmProdottoSeq), 0,
						tp.get(project.dmalmProjectPrimaryKey));
				checkAsmTappo = checkRelazioneParzAsm(
						r.get(0).get(p.dmalmProdottoSeq),
						tp.get(project.dmalmProjectPrimaryKey));
