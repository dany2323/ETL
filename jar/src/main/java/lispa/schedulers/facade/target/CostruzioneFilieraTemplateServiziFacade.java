...
import lispa.schedulers.dao.target.FilieraTemplateServiziDAO;
 ...
 * per il template Servizi: 
 * a. Release Servizi (release_ser)
 * c. Task (task)
 * x. Documento (documento)
 */
public class CostruzioneFilieraTemplateServiziFacade {
	...
	public static void execute() {
		...
		try {
			...
			logger.info("START CostruzioneFilieraTemplateServiziFacade.execute()");
			...
			logger.info("CostruzioneFilieraTemplateServiziFacade - dataInizioFiliera: "
					+ dataInizioFiliera);
			...
			// ricaricata
			// nuovamente
			FilieraTemplateServiziDAO.delete();
			FilieraTemplateDocumentiDAO.delete(DmAlmConstants.SERDEP);

			// creazione filiera template Demand
			List<DmalmLinkedWorkitems> startWorkitemsList = LinkedWorkitemsDAO
					.getStartWorkitemsTemplateServizi(dataInizioFiliera);
			logger.debug("CostruzioneFilieraTemplateServiziFacade - lista.size: "
			...
			logger.debug("CostruzioneFilieraTemplateServiziFacade - Aggiunta WI Build - lista.size: "
			...
			logger.info("STOP CostruzioneFilieraTemplateServiziFacade.execute()");
		...
		for (DmalmLinkedWorkitems linkedWorkitem : linkedWorkitems) {
			...
			if (!figlioPresente(insertedWorkitemsList, linkedWorkitem)) {
				nextWorkitemsList = LinkedWorkitemsDAO
						.getNextWorkitemsTemplateServizi(linkedWorkitem);
			}
	...
	private static void inserisciLista(Integer idFiliera,
		...
		for (DmalmLinkedWorkitems insertedWorkitem : insertedWorkitemsList) {
			...
			FilieraTemplateServiziDAO.insert(idFiliera, livello, sottoLivello,
