				...
				if (target_difettoprodotto.size() == 0) {
					...
					for (Tuple row : target_difettoprodotto) {
						if (row != null) {
							...
							if (modificato && row.get(dif.dmalmProjectFk02)!=0) {
								righeModificate++;
	...
	public static void updateProjectAndStatus(Timestamp dataEsecuzione) {
		...
		try {
			...
			for (DmalmDifettoProdotto difetto : x) {
				...
				// project.getIdProject e data fine validita uguale a 31-12-9999
				difetto_tmp = difetto;
				DifettoDAO.deleteIfNotAnnullato(difetto.getCdDifetto(),difetto.getIdRepository());
			...
			for (DmalmDifettoProdotto difetto : x) {
				...
				// project.getIdProject e data fine validita uguale a 31-12-9999
				difetto_tmp = difetto;
				DifettoDAO.updateProjectAndStatus(difetto);
