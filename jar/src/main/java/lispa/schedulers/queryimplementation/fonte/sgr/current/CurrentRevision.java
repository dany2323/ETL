...
public class CurrentRevision extends com.mysema.query.sql.RelationalPathBase<lispa.schedulers.bean.fonte.sgr.current.CurrentRevision> {

    private static final long serialVersionUID = 1317911173;

    public static final CurrentRevision revision = new CurrentRevision("REVISION");
    ...
    public CurrentRevision(String variable) {
        super(lispa.schedulers.bean.fonte.sgr.current.CurrentRevision.class, forVariable(variable), DmAlmConstants.POLARION_SCHEMA, "REVISION");
    }

    public CurrentRevision(Path<? extends lispa.schedulers.bean.fonte.sgr.current.CurrentRevision> path) {
        super(path.getType(), path.getMetadata(), DmAlmConstants.POLARION_SCHEMA, "REVISION");
    }

    public CurrentRevision(PathMetadata<?> metadata) {
        super(lispa.schedulers.bean.fonte.sgr.current.CurrentRevision.class, metadata, DmAlmConstants.POLARION_SCHEMA, "REVISION");
    }
