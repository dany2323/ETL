 ...
 * 
 */
public class SIREUserRolesXML {

	private static Logger logger = Logger.getLogger(SIREUserRolesXML.class);
	...
	// public static void fillSIRECurrentUserRoles() throws Exception {
	//
	...
	// int numberofusers = QueryUtils
	// .getCountUserRoles(DmAlmConstants.REPOSITORY_SIRE);
	// if (numberofusers != 0) {
	...
	// DmAlmConfigReaderProperties.SIRE_SVN_PASSWORD);
	//
	...
	//
	// SireCurrentProject project = SireCurrentProject.project;
	//
	// List<Tuple> locations = query.distinct().from(project)
	// .list(project.cLocation, project.cPk, project.cId);
	//
	// String projectSVNPath = "";
	// for (Tuple el : locations) {
	//
	// projectSVNPath = getProjectSVNPath(el
	// .get(project.cLocation));
	// if (projectSVNPath != null) {
	// insertUserRoles(DmAlmConstants.REPOSITORY_SIRE,
	// el.get(project.cId), projectSVNPath,
	// el.get(project.cPk), DataEsecuzione
	// .getInstance().getDataEsecuzione());
	// }
	// }
	//
	// insertGlobalUserRoles(DmAlmConstants.REPOSITORY_SIRE);
	// }
	// } catch (Exception e) {
	...
	// String projectLocation, String cpk, Timestamp dataEsecuzione)
	// throws Exception {
	//
	...
	public static void recoverSIREUserRoles() throws Exception {
		ConnectionManager cm = null;
		Connection connection = null;
	...
	public static void fillSIREHistoryUserRoles() throws DAOException {
		Connection oraConn = null;
		ConnectionManager cm = null;

		try {
			// 20150623
			logger.debug("fillSIREHistoryUserRoles - inizio");
			...
			QSireHistoryProject project = QSireHistoryProject.sireHistoryProject;

			SQLQuery query = new SQLQuery(oraConn, dialect);

			// 20150623
			logger.debug("fillSIREHistoryUserRoles - query project");
			...
			// 20150623
			logger.debug("fillSIREHistoryUserRoles - filteredLocations.size: "
					+ filteredLocations.size());

			long minRevision = UserRolesDAO
					.getMinRevisionGlobal(DmAlmConstants.REPOSITORY_SIRE);

			// 20150623
			logger.debug("fillSIREHistoryUserRoles - POST getMinRevision, minRevision: "
					+ minRevision);

			for (Tuple el : filteredLocations) {
				// 20150623
				// logger.debug("fillSIREHistoryUserRoles - getProjectSVNPath clocation: "
				// + project.cLocation);
				...
				// 20150623
				// logger.debug("fillSIREHistoryUserRoles - getMinRevision cid: "
				// + project.cId + ", minRevision: " + minRevision);

				if (projectSVNPath != null) {
					// 20150623
					// logger.debug("fillSIREHistoryUserRoles - start insertUserRolesfromScratch");

					insertUserRolesfromScratch(DmAlmConstants.REPOSITORY_SIRE,
							el.get(project.cId), projectSVNPath,
							el.get(project.cPk), el.get(project.cRev),
							el.get(project.cCreated), minRevision);

					// 20150623
					// logger.debug("fillSIREHistoryUserRoles - stop insertUserRolesfromScratch");
				}
			}

			// 20150623
			logger.debug("fillSIREHistoryUserRoles - start insertGlobalUserRoles");

			insertGlobalUserRoles(DmAlmConstants.REPOSITORY_SIRE);

			// 20150623
			logger.debug("fillSIREHistoryUserRoles - fine");
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
