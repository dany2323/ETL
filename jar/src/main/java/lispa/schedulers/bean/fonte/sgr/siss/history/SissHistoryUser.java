package lispa.schedulers.bean.fonte.sgr.siss.history;

import javax.annotation.Generated;

@Generated("com.mysema.query.codegen.BeanSerializer")
public class SissHistoryUser {

	private String cAvatarfilename;

	private String cDeleted;

	private String cDisablednotifications;

	private String cEmail;

	private String cId;

	private String cInitials;

	private String cIsLocal;

	private String cName;

	private String cPk;

	private Long cRev;

	private String cUri;

	public String getcAvatarfilename() {
		return cAvatarfilename;
	}

	public void setcAvatarfilename(String cAvatarfilename) {
		this.cAvatarfilename = cAvatarfilename;
	}

	public String getcDeleted() {
		return cDeleted;
	}

	public void setcDeleted(String cDeleted) {
		this.cDeleted = cDeleted;
	}

	public String getcDisablednotifications() {
		return cDisablednotifications;
	}

	public void setcDisablednotifications(String cDisablednotifications) {
		this.cDisablednotifications = cDisablednotifications;
	}

	public String getcEmail() {
		return cEmail;
	}

	public void setcEmail(String cEmail) {
		this.cEmail = cEmail;
	}

	public String getcId() {
		return cId;
	}

	public void setcId(String cId) {
		this.cId = cId;
	}

	public String getcInitials() {
		return cInitials;
	}

	public void setcInitials(String cInitials) {
		this.cInitials = cInitials;
	}

	public String getcIsLocal() {
		return cIsLocal;
	}

	public void setcIsLocal(String cIsLocal) {
		this.cIsLocal = cIsLocal;
	}

	public String getcName() {
		return cName;
	}

	public void setcName(String cName) {
		this.cName = cName;
	}

	public String getcPk() {
		return cPk;
	}

	public void setcPk(String cPk) {
		this.cPk = cPk;
	}

	public Long getcRev() {
		return cRev;
	}

	public void setcRev(Long cRev) {
		this.cRev = cRev;
	}

	public String getcUri() {
		return cUri;
	}

	public void setcUri(String cUri) {
		this.cUri = cUri;
	}

}
