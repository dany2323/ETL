...
public class TestCheckAnnullamentiSGR extends TestCase {
	private static Logger logger = Logger
			.getLogger(TestCheckAnnullamentiSGR.class);
...
//	private static String urlSire, urlSiss;
	...
	public static void testCheckAnnullamenti() {

		DmAlmConfigReaderProperties.setFileProperties("/Users/lucaporro/LISPA/DataMart/props/dm_alm.properties");
		DataEsecuzione.getInstance().setDataEsecuzione(DateUtils.stringToTimestamp("2020-03-25 21:00:00.0", "yyyy-MM-dd HH:mm:00"));
		try {
...
//					DmAlmConfigReaderProperties.UNMARKED_PROJECTS_FILE_SISS);
			 ...
			 */
			unmarkedProjectsSire = getUnmarkedProjectsPathsFromFile(DmAlmConstants.REPOSITORY_SIRE, DataEsecuzione.getInstance().getDataEsecuzione());
			unmarkedProjectsSiss = getUnmarkedProjectsPathsFromFile(DmAlmConstants.REPOSITORY_SISS, DataEsecuzione.getInstance().getDataEsecuzione());
			/**
	...
	private static void annullaWorkitemFigli() throws DAOException,
	...
	private static void annullaWorkitemDeleted() throws DAOException,
	...
	private static List<String> getUnmarkedProjectsPathsFromFile(String repo, Timestamp dataEsecuzione) throws IOException, DAOException {

		BufferedReader in = null;

		ConnectionManager cm = null;
		Connection connectionOracle = null;
		List<String> projectsUn = new ArrayList<String>();
		SQLTemplates dialect = new HSQLDBTemplates();
		try {
			...
			projectsUn = query
					.from(projectsUnmarked)
					.where(projectsUnmarked.repository.eq(repo))
					.where(projectsUnmarked.dataCaricamento.eq(dataEsecuzione))
					.list(projectsUnmarked.path);
