SELECT
 ...
 NVL(CASE WHEN at.SIGLA_PRODOTTO is null
       THEN 0
       ELSE ( SELECT p.DMALM_PRODOTTO_PK FROM DMALM_EL_PRODOTTI_ARCH p where p.SIGLA = at.SIGLA_PRODOTTO and p.DT_CARICAMENTO = (SELECT MAX(DT_CARICAMENTO) FROM DMALM_EL_PRODOTTI_ARCH mx where mx.SIGLA = at.SIGLA_PRODOTTO) AND ANNULLATO= 'NO') 
