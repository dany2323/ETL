SELECT
 ...
 NVL(CASE WHEN SIGLA_MODULO is null
       THEN 0
       ELSE ( SELECT m.DMALM_MODULO_PK FROM DMALM_EL_MODULI m where m.SIGLA_MODULO = at.SIGLA_MODULO and m.DT_CARICAMENTO = (SELECT MAX(DT_CARICAMENTO) FROM DMALM_EL_MODULI mx where mx.SIGLA_MODULO = at.SIGLA_MODULO) AND ANNULLATO= 'NO') 
 END, 0) as MODULO_FK 
