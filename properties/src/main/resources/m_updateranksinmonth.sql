...
update DMALM_CLASSIFICATORE_DEMAND set RANK_STATO_CLASSIF_MESE = 0;
...
where ranker = 1); 
update DMALM_CLASSIFICATORE_DEMAND set RANK_STATO_CLASSIF_MESE = 1 where DMALM_CLASSIFICATORE_PK in (
...
select a.DMALM_CLASSIFICATORE_PK, row_number() over 
	...
	order by DMALM_CLASSIFICATORE_PK desc) as ranker from DMALM_CLASSIFICATORE_DEMAND a
) Z 
where ranker = 1); 
