SELECT
 ...
 NVL(CASE WHEN at.SIGLA_PRODOTTO is null
       THEN 0
       ELSE ( SELECT MAX(p.DMALM_PRODOTTO_PK) FROM DMALM_EL_PRODOTTI_ARCH p where p.SIGLA = at.SIGLA_PRODOTTO and p.DT_CARICAMENTO = (SELECT MAX(DT_CARICAMENTO) FROM DMALM_EL_PRODOTTI_ARCH mx where mx.SIGLA = at.SIGLA_PRODOTTO) AND ANNULLATO= 'NO') 
 END, 0) as PRODOTTO_FK,
 NVL(CASE WHEN SIGLA_MODULO is null
       THEN 0
       ELSE ( SELECT MAX(m.DMALM_MODULO_PK) FROM DMALM_EL_MODULI m where m.SIGLA_MODULO = at.SIGLA_MODULO and m.DT_CARICAMENTO = (SELECT MAX(DT_CARICAMENTO) FROM DMALM_EL_MODULI mx where mx.SIGLA_MODULO = at.SIGLA_MODULO) AND ANNULLATO= 'NO') 
 END, 0) as MODULO_FK 
