SELECT  
		...
		hw.c_uri as URI_WI, 
		hw.sire_history_workitem_pk as DMALM_DOCUMENTO_PK,
		hw.c_pk as STG_DOCUMENTO_PK, 
		hw.c_id as CD_DOCUMENTO, 
		...
		hw.c_updated as DATA_MODIFICA_DOCUMENTO,
		...
		) as DATA_RISOLUZIONE_DOCUMENTO, 
		hw.c_resolution as MOTIVO_RISOL_DOCUMENTO,
		hw.c_title as TITOLO_DOCUMENTO, 
		to_char(hw.c_description) as DESCRIZIONE_DOCUMENTO,
        hw.classe_doc as CLASSIFICAZIONE_DOCUMENTO,
	...
	(select distinct to_char(hcf.c_string_value) from dmalm_sire_history_cf_workitem hcf where hcf.fk_workitem = hw.c_pk and hcf.c_name = 'tag_alm') as TAG_ALM,
		...
		where hw.c_type = 'documento'
		and hw.data_caricamento = ? 
UNION ALL  
   SELECT  
		'SISS' as ID_REPOSITORY,
		hw.c_uri as URI_WI, 
		hw.siss_history_workitem_pk as DMALM_DOCUMENTO_PK,
		hw.c_pk as STG_DOCUMENTO_PK, 
		hw.c_id as CD_DOCUMENTO, 
		hw.c_created as DATA_INSERIMENTO_RECORD, 
